...
class GeometricAffine(nn.Module):
    ...
    def forward(self, points, points_features):
        """
        ...
        return sample_points,sample_points_features
...
class PointMLP(nn.Module):
    def __init__(self, points_num=1024, class_num=15):
        ...
        self.embedding = Block(3, 64, bias=False)  # 用于计算所有点的特征
        # 为每个state添加特征提取模块
        ...
        self.pre_blocks_list = nn.ModuleList([
            PreExtraction(128, 128, bias=False),
            PreExtraction(256, 256, bias=False),
            PreExtraction(512, 512, bias=False),
            PreExtraction(1024,1024,bias=False)
        ])
        self.pos_blocks_list = nn.ModuleList([
            PosExtraction(128,bias=False),
            PosExtraction(256,bias=False),
            PosExtraction(512,bias=False),
            PosExtraction(1024,bias=False)
        ])
        # 分类器
    ...
    def forward(self, x):
        ...
        features = self.classifier(features)  # (batch, class_num)
        return features
