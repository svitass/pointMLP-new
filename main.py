...
args.add_argument('--checkpoint', type=str, default='checkpoints/best_checkpoint.pth')
args.add_argument('--batch_size', type=int, default=32)
args.add_argument('--num_classes', type=int, default=15) # 15, 40
args.add_argument('--dataset', type=str, default='ScanObjectNN') # ModelNet40, ScanObjectNN
...
def train(net, train_loader, optimizer, criterion, device):
    ...
    for batch_idx,(data,label) in enumerate(train_loader):
        data, label = data.to(device), label.to(device).squeeze()  # (batch_size, points_num, 3)
        optimizer.zero_grad()
        logits = net(data)
